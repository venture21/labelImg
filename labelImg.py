...
# class MainWindow
#####################################################################
# 2개의 클래스를 다중 상속 받아 파생 클래스를 생성한다.
class MainWindow(QMainWindow, WindowMixin):
    # 3개의 변수를 생성한다.
    FIT_WINDOW, FIT_WIDTH, MANUAL_ZOOM = list(range(3))

    # 클래스 생성자
    # images_filepath : 이미지가 저장된 디렉터리
    # 
    def __init__(self, images_filepath=None, class_filepath=None, label_save_dir=None):
        
        # 부모 클래스(QMainWindow) 초기화
        # WindowMixin 클래스는 __init__이 없음
        super(MainWindow, self).__init__()

        # 창의 이름 설정
        self.setWindowTitle(__appname__)

        # 사용자의 홈디렉토리에서 ".labelImgSettings.pkl" 파일 정보 가져오기
        # pkl파일은 파이썬의 객체 정보를 저장함
        ...
        self.load_predefined_classes(class_filepath)
        ...
        use_default_label_qvbox_layout =QVBoxLayout()
        ...
        # class_filepath 를 QLabel에 출력 
        label = QLabel('{}'.format(class_filepath), self)
        use_default_label_qvbox_layout.addWidget(label)
        use_default_label_qvbox_layout.addLayout(use_default_label_qhbox_layout)
        use_default_label_container.setLayout(use_default_label_qvbox_layout)
        ...
        ##################################################################################
        # Actions : new_action함수는 utils.py파일에 있음
        ##################################################################################
        ...
        ##########################################################################
        # 선택된 포맷의 (title, icon_name)을 반환한다.
        ##########################################################################
        ...
        # Application state.
        self.image = QImage()
        self.file_path = ustr(images_filepath)
        self.last_open_dir = None
        self.recent_files = []
        self.max_recent = 7
        ...
        if self.label_save_dir is None and save_dir is not None and os.path.exists(save_dir):
            self.label_save_dir = save_dir
            self.statusBar().showMessage('%s started. Annotation will be saved to %s' %
                                         (__appname__, self.label_save_dir))
            self.statusBar().show()
    ...
    def show_bounding_box_from_annotation_file(self, file_path):
        if self.label_save_dir is not None:
            basename = os.path.basename(os.path.splitext(file_path)[0])
            xml_path = os.path.join(self.label_save_dir, basename + XML_EXT)
            txt_path = os.path.join(self.label_save_dir, basename + TXT_EXT)
            json_path = os.path.join(self.label_save_dir, basename + JSON_EXT)
    ...
    ##############################################################################
    # 창을 닫을 때 호출되며, 현재 작업 환경 정보를 저장합니다.
    ##############################################################################
    def closeEvent(self, event):
        ...
        if self.label_save_dir and os.path.exists(self.label_save_dir):
            settings[SETTING_SAVE_DIR] = ustr(self.label_save_dir)
        else:
    ...
    def change_save_dir_dialog(self, _value=False):
        ...
        if dir_path is not None and len(dir_path) > 1:
            self.label_save_dir = dir_path

        self.show_bounding_box_from_annotation_file(self.file_path)

        self.statusBar().showMessage('%s . Annotation will be saved to %s' %
                                     ('Change saved folder', self.label_save_dir))
        self.statusBar().show()
    ...
    def open_dir_dialog(self, _value=False, dir_path=None, silent=False):
        ...
        self.label_save_dir = target_dir_path
        if self.file_path:
    ...
    def save_file(self, _value=False):
        if self.label_save_dir is not None and len(ustr(self.label_save_dir)):
            if self.file_path:
                image_file_name = os.path.basename(self.file_path)
                saved_file_name = os.path.splitext(image_file_name)[0]
                saved_path = os.path.join(ustr(self.label_save_dir), saved_file_name)
...
def get_main_app(argv=None):
    ...
    #####################################################################
    # Usage : labelImg.py [image_dir] [class_file] [save_dir]
    #####################################################################
